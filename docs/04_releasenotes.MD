...
## [v1.9.0] - Soft Delete, Build Fixes, DnD Restore - 2025-04-24

### Summary
Implement soft delete, fix build issues after Next.js upgrade attempt (reverted to 14.0.4), restore DnD functionality.

### Features
- **Soft Delete:** Implemented a soft delete mechanism for cards. Clicking the delete icon (🗑️) now marks a card as deleted (`isDeleted: true`, `deletedAt: Date`) instead of removing it permanently.
...
- **Frontend (`page.tsx`, `HomePageClient.tsx`):**
    - Added "Deleted" option to the view mode state and UI toggle.
    - Updated data fetching to call `getDeletedCards` for the "Deleted" view.
    - Added `handleCardDelete` function with optimistic UI update and server action call.
...
- **Commit:** `611c1a5`
- **Platform:** Vercel
- **URL:** `https://doneisbetter-4im0qdwi6-narimato.vercel.app`
...
### UI Change
- **Timestamp Icon:** Replaced the text label "Created (UTC):" with the clock emoji "🕒" for the card creation timestamp display, improving visual compactness.
...
- Improved input component with proper loading states
- Stabilized admin role implementation
- Optimized MongoDB connection handling
- Updated NextAuth configuration for better type safety
## v1.10.2 (2025-04-24)
- Added 'Admin Login (Dev)' button for testing (non-production only)
- Fixed numerous build errors related to TypeScript types, imports, and syntax
- Corrected NextAuth adapter version mismatch and configuration
- Enabled Next.js Server Actions
- Refined MongoDB connection logic
- Stabilized build and deployment process
## v1.10.3 (2025-04-24)
- **Feat:** Automatically assign 'admin' role to 'moldovancsaba@gmail.com' on Google sign-in.
- **Feat:** Implemented fallback default values for card properties in server actions to prevent loading errors from missing data.
- **Fix:** Corrected multiple critical syntax and type errors in NextAuth configuration ().
- **Fix:** Resolved build failures related to dependencies, imports, and component structure.
## v1.10.4 (2025-04-24)
- **Refactor:** Separated client-side logic in  into  component to optimize rendering (addresses build warning, though warning persists due to necessary client hooks).
- **Fix:** Corrected multiple critical syntax errors and structural issues in  resulting from previous merge conflicts.
- **Fix:** Resolved type error in  function call within .
## v1.12.0 (2025-04-24)
- **Feat:** Enhanced Admin Dashboard (/admin) to display users and allow role changes (user/admin).
- **Refactor:** Switched UserTable component to use server actions directly instead of API route.
- **Fix:** Corrected data transformation for user list display.
## v1.13.0 (2025-04-24)
- **Feat:** Added permanent user deletion capability for admins on the Admin Dashboard.
- **Feat:** Includes confirmation dialog and prevents admins from deleting themselves.
- **Feat:** User deletion performs a transaction to remove the user and their associated cards.
- **Fix:** Corrected persistent corruption issues in  component.
## v1.14.0 (2025-04-24)
- **Feat:** Added conditional 'Admin' link in header for users with admin role.
- **Note:** Database cleanup (removing non-admin users) needs to be performed manually due to agent environment limitations.
