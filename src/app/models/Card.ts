...
CardSchema.index({ status: 1, order: 1 }); // For sorting cards by status and order
...
interface CardModel extends Model<CardDocument> {
  /**
   * Finds cards and returns them in a format compatible with the frontend Card type
   */
  findCards(): Promise<Array<{
    id: string;
    content: string;
    status: CardStatus;
    order: number;
...
CardSchema.statics.findCards = async function(): Promise<any[]> {
  const cards = await this.find({}).sort({ status: 1, order: 1 });
...
CardSchema.statics.updateCardStatus = async function(
  ...
  try {
    ...
    // Find and update the card, returning the updated document
    const updatedCard = await this.findByIdAndUpdate(
      id,
      updateData,
